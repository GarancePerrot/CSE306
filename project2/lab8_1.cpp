...
public : 
    ...
    void time_step(double dt){ // slides p.17 : Algorithm at time step t, Explicit Euler

        OT.diagram.points = positions;
        OT.optimize();    // get current OT of particles
        ...
        for (int i = 0; i < positions.size(); i++){   
            ...
            Vector F_i = F_spring + (m_i*g);    // we add forces : spring and gravity
